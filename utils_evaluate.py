...
class EVAL_OPTS:
    def __init__(self, data_file, pred_file, tag_pred_file, result_file='', out_file="",
                 na_prob_file=None, na_prob_thresh=1.0, out_image_dir=None, verbose=False):
...
def compute_coin(f, t_gold, addition, t_pred):
...
def check_answer_pattern(em, f1, th=0.9, outfile='temp7.txt'):
    with open(outfile, 'w') as f:
        for k, v in em.items():
            if th < f1[k] < th + 0.1 and v != 1:
                f.write(str(k) + ' ' + str(f1[k]) + '\n')
...
def main(OPTS):
    ...
    if OPTS.na_prob_file:
        if isinstance(OPTS.na_prob_file, str):
            with open(OPTS.na_prob_file) as f:
                na_probs = json.load(f)
        else:
            na_probs = OPTS.na_prob_file
    else:
        na_probs = {k: 0.0 for k in preds}
    qid_to_has_ans = make_qid_to_has_ans(dataset)  # maps qid to True/False
    has_ans_qids = [k for k, v in qid_to_has_ans.items() if v]
    no_ans_qids = [k for k, v in qid_to_has_ans.items() if not v]
    exact_raw, f1_raw, coin_raw = get_raw_scores(dataset, preds, tag_preds, data_dir)
    exact_thresh = apply_no_ans_threshold(exact_raw, na_probs, qid_to_has_ans,
                                          OPTS.na_prob_thresh)
    f1_thresh = apply_no_ans_threshold(f1_raw, na_probs, qid_to_has_ans,
                                       OPTS.na_prob_thresh)
    coin_thresh = apply_no_ans_threshold(coin_raw, na_probs, qid_to_has_ans,
                                         OPTS.na_prob_thresh)
    # check_answer_pattern(exact_thresh, f1_thresh)
    out_eval = make_eval_dict(exact_thresh, f1_thresh, coin_thresh)
    if has_ans_qids:
        has_ans_eval = make_eval_dict(exact_thresh, f1_thresh, coin_thresh, qid_list=has_ans_qids)
        merge_eval(out_eval, has_ans_eval, 'HasAns')
    if no_ans_qids:
        no_ans_eval = make_eval_dict(exact_thresh, f1_thresh, coin_thresh, qid_list=no_ans_qids)
    ...
    for p in pages_list:
        ...
        page_eval = make_eval_dict(exact_thresh, f1_thresh, coin_thresh, qid_list=pages_ans_qids)
        merge_eval(write_eval, page_eval, p)
    if OPTS.result_file:
        with open(OPTS.result_file, 'w') as f:
            w = {}
            for k, v in qid_to_has_ans.items():
                w[k] = {'exact': exact_thresh[k], 'f1': f1_thresh[k], 'coin': coin_thresh[k]}
            json.dump(w, f)
    if OPTS.out_file:
